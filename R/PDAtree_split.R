#' Find tree structure using PDA projection pursuit indice of classification in each split.
#'
#' @usage PDAtree_split(fr,data, weight = TRUE,std=TRUE,size.p=0.9, lambda=lambda,...) 
...
#' @param lambda weight flag using in LDA index?
...
#' Tree.result <- PDAtree_split(as.formula('Species~.'), weight=TREUE,data=data1 ,size.p=0.9,lambda=1)
#' 
PDAtree_split <- function (fr,data, weight = TRUE,std=TRUE,size.p=0.9, lambda=lambda,...) 
{
  ...
  Find.proj <- function(i.class, i.data, lambda, ...) {
    ...
    if (G == 2) {
      ...
      class <- rep(0, n)
      for (i in 1:split) class <- class + (as.numeric(factor(i.class)) == 
                                             m.name[i])
      class <- 2 - class
      g <- table(class)
