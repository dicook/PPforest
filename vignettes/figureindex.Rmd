...
Histograms of the most interesting projected data and the least interesting one


```{r,fig.show='hold',fig.width = 6 ,fig.height = 4,echo=FALSE ,message=FALSE}
ilda.best <- ilda[wma]
proj.data.best <- data.frame(Sim=dat.sim[,1],proj=cos(wma*pi/180)*dat.sim[,2]+sin(wma*pi/180)*dat.sim[,3])
proj.data.nobest <- data.frame(Sim=dat.sim[,1],proj=cos(wmi*pi/180)*dat.sim[,2]+sin(wmi*pi/180)*dat.sim[,3])
ggplot(proj.data.best,aes(x=proj))+geom_histogram()
ggplot(proj.data.nobest,aes(x=proj))+geom_histogram()
