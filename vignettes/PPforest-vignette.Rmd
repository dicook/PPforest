...
```{r}
training.id <- PPforest::train_fn(y = crab[, 1],  size.p = 2/3)
...
```{r}
set.seed(146)
pprf.crab <- PPforest::PPforest(y = crab[, 1], x = crab[, -1], size.tr = 2/3, m = 500, size.p =  .7,  PPmethod = 'LDA', strata = TRUE)
...
PPforest::ppf_importance(y = crab[, 1], x = crab[, -1], pprf.crab, global = TRUE, weight = TRUE) 
```
...
pprf.nci60 <- PPforest::PPforest(y = NCI60[, 1], x = NCI60[, -1], size.tr = 2/3, m = 500, size.p = .7, PPmethod = 'PDA', strata = TRUE, lambda=.5)
pprf.nci60
```
