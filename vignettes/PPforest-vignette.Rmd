...
library(ggplot2)
crab2 <- subset(crab, select=c(Type,RW,BD))
grilla <- base::expand.grid(RW=seq(6,21,,100), BD=seq(6,22,,100))
...
rpart.crab <- rpart::rpart(Type ~ RW + BD, data=crab2)
...
p<- ggplot2::ggplot(data = grilla ) + ggplot2::geom_point(ggplot2::aes(x =RW, y = BD  , color = as.factor(ppred)),alpha = .5) + ggplot2::scale_shape_discrete(name = "Type") + ggplot2::scale_colour_discrete(name  ="Type.bound") 
pl.pp <- p + ggplot2::geom_point(data = crab2, ggplot2::aes(x =RW , y = BD, shape = as.factor(Type)), size = I(2)  ) +  ggplot2::theme(legend.position = "none")

 p2 <- ggplot2::ggplot(data = grilla ) + ggplot2::geom_point(ggplot2::aes(x =RW, y = BD  , color = as.factor(rpart.pred)),alpha = .5) + ggplot2::scale_shape_discrete(name = "Type") + ggplot2::scale_colour_discrete(name  ="Type.bound") 
 pl.rpart <- p2 + ggplot2::geom_point(data = crab2, ggplot2::aes(x =RW , y = BD, shape = as.factor(Type)), size = I(2)  ) +  ggplot2::theme(legend.position = "none")

gridExtra::grid.arrange(pl.rpart,pl.pp, nrow=1)
```
The right panel shows the ```PPtree``` decision boundaries while the left panel shows the ```rpart``` decision boundaries.
As we mention before we can observe that for this case the decision boundaries from  ```rpart``` (orthogonal to the axis) are not apropriate to the characteristics of this data set while the boundaries form ```PPtree``` are more adecuate for our correlated data.
...
The projection coefficient of the node represent the variable importance.
...
```
In this example we can observe that trees from ```PPtree``` are smaller and simpler to ```rpart```.
...
4.  Predict the classes of each case not included in the bootstrap sample.
5. Based on majority vote predict the class
...
```PPforest``` function runs a projection pursuit random forest,  using this function we have the option to split the data in training and test using size.tr. ```size.tr``` is the size proportion of the training then the proportion of this will be 1- ```size.tr```.
...
We can also plot the MDS using proximity matrix information with the function \code{pproxy_plot}.
...
We can explore the optimal projection for classification in q-dimensional space. We select q=1 and  PDA  projection pursuit indexe.
...
```
We can see that leukemia cases are separated from the rest.

```{r}
set.seed(143)
pprf.nci60 <- PPforest::PPforest(data = NCI60, size.tr = 2/3, m = 500, size.p = .2, PPmethod = 'PDA', strata = TRUE, lambda=.3)
...
```{r,fig.show='hold',fig.width = 6 ,fig.height = 6,echo=FALSE}
PPforest::ppf_importance(NCI60, pprf.nci60, global = TRUE, weight = TRUE) 
```
