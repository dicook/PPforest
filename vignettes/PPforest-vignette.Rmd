...
The PPforest function runs a projection pursuit random forest, the argument testap=TRUE allows to use test data. m is the number of trees in the forest size.p is the sample proportion of the variables used in each node split, PPmethod is the progection pursuit index to be optimized there are two options LDA and PDA. Finally strata=TRUE indicate that the bootstrap samples are stratified by class variable. 
...
We cancompute a global importance measure for the forest using the function importance.
```{r}
PPforest::importance(training.crab, pprf.crab,global=TRUE, weight=TRUE) 
```

We can see that the most important variable in this example is RW while the less important is CW
