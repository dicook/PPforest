...
p <- ggplot2::ggplot(data = grilla ) + ggplot2::geom_point(ggplot2::aes(x =RW, y = BD  , color = as.factor(ppred)),alpha = .20)+ ggplot2::scale_colour_brewer(type="qual",palette="Set1") + ggplot2::theme_bw() 
  


pl.pp <- p + ggplot2::geom_point(data = crab2, ggplot2::aes(x =RW , y = BD, group=Type,shape = Type, color=Type), size = I(3)  ) +  ggplot2::scale_shape_manual(name = "Class", labels = levels(crab2$Type),values = c(19, 18, 15, 17)) + ggplot2::scale_colour_manual(name = "Class",
                      labels = levels(crab2$Type),
                      values = c("#e41a1c", "#377eb8", "#4daf4a", "#984ea3")) + theme(legend.position = "bottom", legend.text = element_text(size = 6))


 p2 <- ggplot2::ggplot(data = grilla ) + ggplot2::geom_point(ggplot2::aes(x =RW, y = BD  , color = as.factor(rpart.pred)), alpha = .2) + ggplot2::scale_colour_brewer(type="qual",palette="Set1") + ggplot2::theme_bw() 
...
set.seed(146)
grilla <- data.frame(FL=runif(10000)*((max(crab$FL)-min(crab$FL))+min(crab$FL)),RW=runif(10000)*((max(crab$RW)-min(crab$RW))+min(crab$RW)), CL=runif(10000)*((max(crab$CL)-min(crab$CL))+min(crab$CL)), CW=runif(10000)*((max(crab$CW)-min(crab$CW))+min(crab$CW)),BD=runif(10000)*((max(crab$BD)-min(crab$BD))+min(crab$BD)))

pr <- predict(pprf.crab,test.data=grilla)
cond <- pr[[1]] > 0.7
cond2 <- apply(cond, 1, sum)


datpp <- data.frame(pred = pr[[2]], grilla)
datpp2 <- datpp[!cond2,]
...
colnames(crab)[1]<-"pred"
aux<-rbind(datpp2,crab)
aux$dat<-c(rep(0,7810),rep(1,200))

princ <- princomp(datpp2[,-1])

ggplot(data.frame(princ$scores), aes(x = Comp.1, y = Comp.2, color = datpp2[,1])) + geom_point()
#save(pprf.crab,file="datpp.Rdata")
write.csv(aux,file="aux.csv")
str(pprf.crab, max.level = 1)


p <- ggplot2::ggplot(data = grilla[!cond2,] ) + ggplot2::geom_point(ggplot2::aes(x =RW, y = BD  , color = as.factor(pr[[2]][!cond2])),alpha = .20)+ ggplot2::scale_colour_brewer(type="qual",palette="Set1") + ggplot2::theme_bw() 
  


pl.pp <- p + ggplot2::geom_point(data = crab, ggplot2::aes(x =RW , y = BD, group=Type,shape = Type, color=Type), size = I(3)  ) +  ggplot2::scale_shape_manual(name = "Class", labels = levels(crab2$Type),values = c(19, 18, 15, 17)) + ggplot2::scale_colour_manual(name = "Class",
                      labels = levels(crab2$Type),
                      values = c("#e41a1c", "#377eb8", "#4daf4a", "#984ea3")) + theme(legend.position = "bottom", legend.text = element_text(size = 6))
...
gpd <- GGally::ggparcoord(data = NCI60, columns = 2:31, groupColumn = 1, order = "anyClass", showPoints = TRUE) + theme(axis.text.x = element_text(angle = 90), legend.position="bottom")
 gpd
```
