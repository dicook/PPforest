...
```{r,collapse=TRUE}
set.seed(123)
 training <- train_fn(class=iris[,5],size.p=0.7)
...
```{r}
 ...
 data1<-iris[,5:1]
 Tree.result <- PPtree_split(as.formula('Species~.'), data=data1 ,  size.p=0.9)
...
```{r, collapse=TRUE, cache=TRUE, fig.width = 5 ,fig.height = 3}
 grilla <-expand.grid(x3=seq(0,10,,100), x4=seq(0,10,,100), x1=3, x2=5.8)
 colnames(grilla)<-colnames(train)[-1]
 pred <- forest_ppred( grilla , ppfr.iris[[7]] )
 grilla$pred<-pred[[3]]
...
```{r, collapse=TRUE, cache=TRUE, fig.width = 5 ,fig.height = 3}
 grilla <- expand.grid(x3=1.3, x4=4.4, x1=seq(0,10,,100), x2=seq(0,10,,100))
 colnames(grilla)<-colnames(train)[-1]
 pred <- forest_ppred( grilla , ppfr.iris[[7]] )
 grilla$pred<-pred[[3]]
