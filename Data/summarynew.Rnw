...
result.bagg.st <- llply(result.boot.st[1:5],function(x) bagging_pp(data=data.iris,boot=x,size.p=.9,training=training))
result.bagg <- llply(result.boot[1:5],function(x) bagging_pp(data=data.iris,boot=x,size.p=.9,training=training))
...
<<olive2,depenson='olive',cache=TRUE,echo=FALSE>>=
